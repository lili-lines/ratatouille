...
The breadboard was for trying, the mouse needs something that survives vibrations. So, the different ways to connect everything together - the boards - and how to make the paths, to choose one.
...
You cut instead of drawing[^1]. Before, the holes of a strip are already connected to each other:
●───●───●───●───●───●───●
...
Same result, made by a factory[^2]. You draw the paths in a software (KiCad[^3]), send the file, and receive the boards a week later: tracks, holes, solder mask, text.
...
Two kinds of wire for the paths, chosen by the current they carry: <br>
. Hookup wire (24 AWG): thick enough for amps, 🐭 the motor wires, 1-3 A. <br>
. Wire-wrap wire (30 AWG): very thin, so it melts above a few hundred mA, 🐭 only for signals, sensors and buttons, a few mA. Fragile, so tape or glue it in place. <br>
For simplicity, the v1 uses 24 AWG everywhere.

<br>

For the v1 I chose the perfboard, the simplest to start with. A PCB will come in a later version. Now, the structure that will carry the electronics.

<br>

[^1]: [Wikipedia — Stripboard](https://en.wikipedia.org/wiki/Stripboard)
[^2]: [Wikipedia — Printed circuit board](https://en.wikipedia.org/wiki/Printed_circuit_board)
[^3]: [KiCad](https://www.kicad.org/), the open source software to draw a PCB

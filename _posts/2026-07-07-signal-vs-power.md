...
| Encoding | The info is in… | Who reads it | 🐭 In the mouse |
|---|---|---|---|
| Logic | voltage: 3.3V=1, 0V=0 | a digital pin | wheel encoders |
| Analog | value of the voltage, 1.6 V = a given angle | an ADC | AS5600 → 0–4095 |
| Protocol (I2C, PWM…) | a coded sequence of pulses | dedicated hardware in the chip | MPU-6050 gyro (I2C), motor speed (PWM) |
...
#### 1.3 Common GND ⚫
GND it is the shared (−) signals and power are both (+). Separated on the (+) side, they meet on the (−) side.
...
E.g. a 20 cm Dupont wire is about 0.03 Ω: <br>
. power wires → motor → 3A → 0.03x3=90mV → must be short and thick <br>
. signal wires → AS5600 → 0.03A → 0.03x0.03=0.9mV → tolerant <br>
The same wire, 100× difference, depending on the current going through it. The length matters as much for the (+) as for the (-).

Everybody shares the same ground, means this is where the noise can travel. That's why the GND wires all meet at one precise rendez-vous point: no wire length left, so no R, so no path left for the noise.

To illustrate, take a measurement of the AS5600 read by the Teensy: measurement = signal - reference, the signal comes from the AS5600 and the error = GND of the AS5600 - GND of the Teensy, the bigger that gap, the more wrong the measurement.

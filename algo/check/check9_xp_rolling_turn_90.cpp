...
// virage en roulant (pas un pivot) : les 2 roues avancent, une plus lentement
// que l'autre, ce qui fait courber la trajectoire -- comme une voiture qui tourne
// s'arrete quand le cap atteint 90 (gyro)
...
const int OUTER_SPEED = 130;  // roue exterieure (gauche, B), 0-255
const int INNER_SPEED = 50;   // roue interieure (droite, A), plus lente -> courbe
...
void setup() {
  ...
  Serial.println("Pose le robot, espace degage devant. GO pour le virage en roulant.");
}
...
void loop() {
  ...
  if (digitalRead(BTN_GO) == LOW && state == IDLE) {
    ...
    lastTime = millis();
    state = TURNING;
    motorA(-INNER_SPEED);  // droite, roue interieure -- plus lente
    motorB(OUTER_SPEED);   // gauche, roue exterieure -- plus rapide
    Serial.println("GO -> virage en roulant vers la droite...");
    delay(300);
  }
  ...
  if (state == TURNING) {
    ...
    if (abs(headingDeg) >= TARGET_ANGLE) {
      ...
      Serial.println(headingDeg);
      Serial.println("Mesure le rayon de courbure reel. RESET pour recommencer.");
    }
    delay(20);
  }
}
